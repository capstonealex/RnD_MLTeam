...
checkDirectoryRAW_Karo()
checkMakeDirectoryIntent_Karo()
...
for name in all_filepaths:
    ...
    # - Found stationary State
        ...
        # - Automate naming and saving file to directory
            f = open(DIR_K_INTENT+"Exp_"+str(intent_ID)+"-"+intent+"-SR_"+str(SAMPLE_RATE)+"-RT_"+str(RECORD_TIME)+".csv", "w")
            f.write(csvOutput)
            f.close()
